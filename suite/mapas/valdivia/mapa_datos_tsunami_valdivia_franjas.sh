...
# datos run up
more ${datatsunami} | sed 's/nan/0/g' | awk -F"," '{print $2, $3, $4}' | gmt psxy -J -R -P -V -Cpaletarunup.cpt -Sc0.4c -W0.2p,0 -O -K >> ${psfile}
...
total_datos=$(more ${datatsunami} | sed 's/nan/0/g' | awk -F',' '{if ($2!=0 && $3!=0) print $2, $3, $4 }' | wc -l)
# inicializar array
...
datos_xfranja[$i]=$(more ${datatsunami} | awk -F',' -v var=$lim1 -v var2=$lim2 '{if ($3<var && $3>=var2) print  $3 }' | wc -l)
...
datos_fuera=$(more ${datatsunami} | sed 's/nan/0/g' | awk -F',' -v var=$limnorte -v var2=$limsur '{if ($3!=0 && $3>var || $3<=var2) print  $3 }'  | wc -l)
