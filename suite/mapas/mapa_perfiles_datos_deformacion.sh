...
##################################
# nombre archivo Def. a graficar #
##################################
deffile="slip_modelo_maxpdf_valdivia.csv"
...
##########################################
# factor de conversion para suma de Def. #
##########################################
factor="100"
...
paletadeformacion="/home/rodrigo/Documents/varios/cpt/subtle.cpt"    #Paleta de colores a usar
...
#################
# graficar deformacion #
#################
more ${deffile} | awk -F","  '{printf"%2.8f %2.8f %2.8f\n",  $1, $2, $3}' | gmt triangulate -G${deformacion}.grd -I0.1m/0.1m -R -V
gmt makecpt -C${paletadeformacion} -T0/50/0.1 -Z > paletadeformacion.cpt
...
# escala de colores #
#####################
gmt psscale -D7.0c/-1.3c/14.4c/0.5ch -Cpaletadeformacion.cpt -Ba5f1:"Deformation [m]": -O -K -V >> ${psfile}
...
END
# label Def.
echo 1 -48.6 "Def. profile" | gmt pstext -J -R -P -V -F+f12p,Helvetica-Bold,black+jLM -N -O >> ${psfile}
